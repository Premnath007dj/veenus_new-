...
const Button = ({ children, onClick, className, ...props }) => (
  <button 
    onClick={onClick} 
    className={`bg-energetic-green hover:bg-energetic-green transition-colors duration-300 inline-flex items-center justify-center font-semibold ${className}`} 
    {...props}
  >
    {children}
  </button>
...
const HeroSection = ({ onContactClick }) => {
  ...
  useEffect(() => {
    ...
    const maxDist = 120;
    const lineColor = 'rgba(0, 200, 83, 0.25)';
    const dotColor = 'rgba(245, 245, 245, 0.5)';

    const tick = () => {
      ctx.clearRect(0, 0, width, height);
      // The solid background fill is removed to allow the CSS gradient to show through.
  ...
  return (
    <section
      id="hero"
      className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-deep-charcoal to-energetic-green/20 overflow-hidden pt-20"
    >
      {/* Animated low-poly particles canvas */}
      <div className="absolute inset-0">
        <canvas ref={canvasRef} className="w-full h-full block" />
      </div>
      <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-transparent via-deep-charcoal/40 to-deep-charcoal" />
      
      {/* Decorative background elements */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-[2]">
        <div className="absolute top-10 left-5 sm:top-20 sm:left-10 w-2 h-2 bg-energetic-green rounded-full animate-pulse opacity-60" style={{ animationDelay: '0s' }} />
        <div className="absolute top-20 right-10 sm:top-40 sm:right-20 w-1 h-1 bg-electric-blue rounded-full animate-pulse opacity-40" style={{ animationDelay: '1s' }} />
        <div className="absolute bottom-32 left-1/4 w-1.5 h-1.5 bg-energetic-green rounded-full animate-pulse opacity-50" style={{ animationDelay: '2s' }} />
        <div className="absolute bottom-20 right-1/3 w-1 h-1 bg-electric-blue rounded-full animate-pulse opacity-30" style={{ animationDelay: '0.5s' }} />
        
        <div className="absolute top-16 right-16 w-16 h-16 opacity-10">
          <svg viewBox="0 0 24 24" className="w-full h-full text-energetic-green animate-spin-slow">
        ...
        <div className="absolute bottom-20 left-20 w-12 h-12 opacity-8">
          <svg viewBox="0 0 24 24" className="w-full h-full text-electric-blue animate-spin-slower">
            <path d="M12 15.5A3.5 3.5 0 0 1 8.5 12A3.5 3.5 0 0 1 12 8.5a3.5 3.5 0 0 1 3.5 3.5a3.5 3.5 0 0 1-3.5 3.5M19.43 12.97c.04-.32.07-.64.07-.97s-.03-.66-.07-1l2.11-1.63c.19-.15.24-.42.12-.64l-2-3.46c-.12-.22-.39-.31-.61-.22l-2.49 1c-.52-.39-1.06-.73-1.69-.98l-.37-2.65A.506.506 0 0 0 14 2h-4c-.25 0-.46.18-.5.42l-.37 2.65c-.63.25-1.17.59-1.69.98l-2.49-1c-.22-.09-.49 0-.61.22l-2 3.46c-.13.22-.07.49.12.64L4.57 11c-.04.34-.07.67-.07 1s.03.65.07.97l-2.11 1.66c-.19.15-.25.42-.12.64l2 3.46c.12.22.39.3.61.22l2.49-1.01c.52.4 1.06.74 1.69.99l.37 2.65c.04.24.25.42.5.42h4c.25 0 .46-.18.5-.42l.37-2.65c.63-.26 1.17-.59 1.69-.99l2.49 1.01c.22.08.49 0 .61-.22l2-3.46c.12-.22.07-.49-.12-.64l-2.11-1.66Z" fill="currentColor"/>
          </svg>
        </div>
      </div>
            ...
            <div className="max-w-xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center lg:justify-start items-center animate-fade-in" style={{ animationDelay: `${(words.length + 1) * 0.15}s` }}>
                <Button
                  ...
                  className="text-lg sm:text-xl px-8 sm:px-12 py-4 sm:py-5 rounded-xl shadow-lg text-white transform transition-all duration-300 hover:scale-105 hover:shadow-energetic-green/20"
                >
      ...
      <button
        ...
        className="absolute bottom-6 left-1/2 -translate-x-1/2 text-electric-blue/80 hover:text-electric-blue transition-colors z-20"
