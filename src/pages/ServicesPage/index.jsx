...
const ServicesPage = () => {
  ...
  const servicesData = [
    {
      ...
      icon: 'Settings',
      services: [
        { name: 'Competitive & Performance Benchmarking', description: 'We analyze the competitive landscape to position your product for optimal performance and cost-effectiveness.', details: 'We provide a collaborative analysis of competitor products, evaluate key performance metrics, and deliver actionable recommendations to give you a market edge.' },
        { name: 'PMSM, BLDC & SynRM Motor Simulation', description: 'We provide motor simulation using FEMM, back EMF, and thermal analysis to ensure your designs are fully optimized.', details: 'We leverage advanced FEA for in-depth magnetic field analysis, thermal modeling, and performance prediction, ensuring your motor design is optimized from the start.' },
        { name: 'Prototype Electric Motor Development', description: 'We specialize in prototype development, offering an agile, end-to-end service from concept to a validated prototype.', details: 'Our collaborative process covers the complete design lifecycle, including mechanical design, electromagnetic optimization, and thermal management for your custom motor.' },
    ...
    },
    {
      ...
      icon: 'Cpu',
      services: [
        { name: 'Web and App Development', description: 'We provide end-to-end web and mobile app development, delivering scalable, secure, and user-friendly solutions.', details: 'We offer responsive and SEO-friendly websites, intuitive iOS and Android apps with smooth UX/UI, secure backend and API integration, and ongoing maintenance and support.' },
      ]
    }
  ];

  const motorData = [
    { name: 'BLDC Motors', image: '/veenus_new-/assets/images/bldc.png', description: 'High efficiency and reliability, ideal for applications requiring precise control and long operational life.' },
    { name: 'PMSM Motors', image: '/veenus_new-/assets/images/pmsm.png', description: 'Excellent power density and dynamic performance, suitable for high-performance industrial applications.' },
    { name: 'SynRM Motors', image: '/veenus_new-/assets/images/synrm.png', description: 'Known for their robust design, high efficiency, and cost-effectiveness, particularly in variable speed drives.' },
  ];
  ...
  return (
    <>
      <Header />
      {/* Background canvas and gradient overlay */}
      ...
      <main className="relative z-10 pt-20">
        {/* Hero Section */}
        <section className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-[#212121] via-[#212121] to-[#00C853]">
          {/* Animated background elements */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className="absolute top-20 left-10 w-4 h-4 bg-white/20 rounded-full animate-pulse hidden md:block" />
            <div className="absolute top-40 right-20 w-2 h-2 bg-light-green/30 rounded-full animate-pulse hidden md:block" />
            <div className="absolute bottom-32 left-1/4 w-3 h-3 bg-white/15 rounded-full animate-pulse" />
            <div className="absolute bottom-20 right-1/3 w-2 h-2 bg-light-green/25 rounded-full animate-pulse" />

            {/* --- RESPONSIVE MOTOR/GEAR ANIMATION --- */}
            <div className="absolute top-12 right-4 w-12 h-12 lg:top-16 lg:right-16 lg:w-20 lg:h-20 opacity-10">
            ...
            <div className="absolute bottom-16 left-4 w-10 h-10 lg:bottom-20 lg:left-20 lg:w-16 lg:h-16 opacity-8">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center text-white">
              <h1 className={`text-4xl md:text-6xl lg:text-7xl font-bold mb-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              ...
              <p className={`text-lg md:text-xl text-gray-200 max-w-3xl mx-auto mb-12 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        {/* Carousel Section */}
        <section className="py-16 md:py-24 bg-soft-cool-gray">
        ...
        <section id="services-content" className="py-16 md:py-24 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
            <div className="grid lg:grid-cols-4 gap-8">
              ...
              <aside className="lg:col-span-1">
                <div className="lg:sticky lg:top-24 bg-white/10 backdrop-blur-sm border border-light-green rounded-xl p-6 shadow-soft">
                  <h3 className="text-xl font-bold text-dark-blue mb-4">Categories</h3>
                  <ul className="space-y-2">
                    <li>
                      <button
                        onClick={() => setActiveCategory('all')}
                        className={`w-full text-left px-3 py-2 rounded-lg transition-all duration-300 flex items-center justify-between group ${activeCategory === 'all' ? 'bg-primary-green text-white' : 'hover:bg-light-green/40 text-dark-blue'}`}
                      >
                    ...
                    {servicesData.map(cat => (
                      <li key={cat.id}>
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg transition-all duration-300 flex items-center justify-between group ${activeCategory === cat.id ? 'bg-primary-green text-white' : 'hover:bg-light-green/40 text-dark-blue'}`}
                        >
                          <span>{cat.title}</span>
              ...
              <main className="lg:col-span-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {filteredServices.map((service, index) => (
                    <div
                      key={index}
                      className={`group relative bg-white/10 backdrop-blur-sm border border-light-green rounded-xl p-6 shadow-soft transition-all duration-500 hover:-translate-y-2 hover:shadow-lg cursor-pointer ${expandedCard === index ? 'md:col-span-2' : ''} animate-fade-in`}
                    ...
                    >
                      ...
                      {expandedCard === index && (
                        <div className="mt-4 pt-4 border-t border-light-green animate-fade-in">
                          <p className="text-dark-blue">{service.details}</p>
                          <Link to="/contact" className="text-primary-green font-semibold mt-4 inline-flex items-center hover:underline">
                            Get in touch
                          </Link>
                        </div>
                      )}
